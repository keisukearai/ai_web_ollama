...
| CPU/メモリ監視 | ヘッダーに3秒ごとのリアルタイム表示（CPU%・MEM GB）|
| ピーク負荷記録 | AI生成中のCPU/メモリピーク値をDB保存・各メッセージに表示 |
| タイムアウト | 120秒で自動キャンセル、エラーメッセージ表示 |
...
# runner の PID を kill（serve プロセスは残す）
kill -9 <runner の PID>

# CPU が下がったか確認
top -bn1 | grep -E "^%Cpu|ollama"
```

### CPU/メモリ監視

UI ヘッダーに 3 秒ごとのリアルタイム表示（`/api/stats/` エンドポイント）。  
AI 生成中のピーク CPU/メモリ使用率は DB に保存され、各メッセージと管理画面で確認できる。
...
### サーバースペックに合うモデルの目安（6コア・12GB RAM）

| モデル | サイズ | 応答速度 | 用途 |
|---|---|---|---|
| gemma3:4b | 3.3GB | 30〜60秒 | 汎用・日本語 |
| qwen3:4b | 2.5GB | 30〜60秒 | 汎用・バランス良好 |
| qwen2.5-coder:7b | 4.7GB | 60〜120秒 | コード生成特化 |
| llama3.2:3b | 2.0GB | 20〜40秒 | 軽量・高速 |

- **7B以上**のモデルは動作するが遅い（120秒タイムアウトに引っかかる場合あり）
- 複数モデルを入れておくと UI のドロップダウンから切り替え可能
